...
class Model:
    ...
    def train(self):
        ...
        dir = self.c.storage.ROOTDIR / 'data/processed/'
        X = pd.read_pickle(f'{dir}X.pickle')
        X_test = pd.read_pickle(f'{dir}X_test.pickle')
        y = pd.read_pickle(f'{dir}y.pickle')

        result_dict_lgb = self.train_model_classification(X=X,
                                                          X_test=X_test,
                                                          y=y,
                                                          params=params,
                                                          folds=folds,
        ...
        sub = pd.read_csv(f'{self.c.storatge.ROOTDIR}data/raw/sample_submission.csv')
        sub['isFraud'] = result_dict_lgb['prediction']
        sub.to_csv(f'{self.c.storage.ROOTDIR}data/processed/submission.csv', index=False)
    ...
    @timer
    def train_model_classification(self, X, X_test, y,
                                   params,
                                   folds,
                                   model_type='lgb',
                                   eval_metric='auc',
        ...
        for fold_n, (train_index, valid_index) in enumerate(folds.split(X)):
            logger_main.info(f'Fold {fold_n + 1} started at {time.ctime()}')
            ...
            if model_type == 'sklearn':
                ...
                logger_main.info(f'Fold {fold_n}. {eval_metric}: {score:.4f}.')
        ...
        logger_main.info(f'CV mean score: {np.mean(scores):.6f}, std: {np.std(scores):.6f}.')
